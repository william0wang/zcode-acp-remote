...
// Global settings, shown OUTSIDE any session (entry screen): language and the
// destructive server reset. Session-scoped controls live in SessionPanel.
    ...
    <PanelShell title={t("panel.title")} onClose={onClose}>
      <div className="space-y-5 px-4 pb-[max(var(--safe-bottom),1rem)] pt-2">
        ...
        <div>
          ...
          {profile && (
            <p className="truncate pb-1 font-mono text-xs text-faint">{profile.hubUrl}</p>
          )}
          <p className="pb-2 text-[11px] text-faint">{t("panel.changeServerHint")}</p>
          <button
            onClick={() => (confirmForget ? forgetHub() : setConfirmForget(true))}
          ...
          >
            {confirmForget ? t("panel.changeServerConfirm") : t("panel.changeServer")}
          </button>
        </div>
      </div>
    </PanelShell>
