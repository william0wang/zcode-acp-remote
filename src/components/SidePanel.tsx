...
// Right slide-over for everything that is not session switching: session
// config (model / mode / thought), the plan quota card, and language. The
// left drawer stays a pure session list.
export function SidePanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end bg-black/50" onClick={onClose}>
      <aside
        className="flex h-full w-80 max-w-[85%] flex-col overflow-y-auto border-l border-hairline bg-surface pt-[max(env(safe-area-inset-top),0.75rem)] text-ink"
      ...
      >
        ...
        <div className="mt-auto border-t border-hairline p-4 pb-[max(env(safe-area-inset-bottom),1rem)]">
