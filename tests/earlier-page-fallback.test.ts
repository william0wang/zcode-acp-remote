...
// OLD-bridge fallback (companion to earlier-page-routing.test.ts): against a
// bridge older than 0.44.1 there are no page markers, so the collection
// window buffers EVERYTHING — pages still prepend correctly (the live-update
// race is the pre-marker behavior, accepted for old bridges).
  ...
  send(data: string): void {
    ...
    if (frame.method === "initialize") {
      this.reply(frame.id, {
        protocolVersion: 1,
        agentCapabilities: {},
        agentInfo: { name: "zcode-acp-server", version: "0.44.0" },
      });
...
test("an old bridge (no markers) falls back to buffering the whole window", async () => {
  ...
  // Fallback: the whole window rode the buffer, so the page prepends and the
  // live update rides it on top of SEED (the pre-marker behavior).
  expect(idx("OLD-PAGE")).toBeLessThan(idx("LIVE-TAIL"));
  expect(idx("LIVE-TAIL")).toBeLessThan(idx("SEED"));
});
